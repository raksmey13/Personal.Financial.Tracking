...
const BudgetPage = ({ categories: propCategories = [] }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FD] dark:bg-[#0B0F17] p-8 relative transition-colors">
  <div className="max-w-4xl mx-auto space-y-8">

    {/* Header Section */}
    <div className="flex items-center justify-between bg-white dark:bg-[#151D2A] px-6 py-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
    ...
    <div className="space-y-6">
      {safeBudgets.length === 0 ? (
        ...
        if (item.strategy_type === "master_allocation") {
          ...
          return (
            <div key={item.id} className="bg-white dark:bg-[#151D2A] p-6 rounded-3xl shadow-md border border-gray-100 dark:border-gray-800 space-y-4 border-l-8 border-l-blue-500 animate-in fade-in duration-200 transition-colors">
              ...
              <div className="space-y-4 pt-2">
                {item.buckets && item.buckets.map((bucket, idx) => {
                  ...
                  return (
                    <div key={idx} className="space-y-1.5 bg-gray-50/50 dark:bg-[#1E293B]/50 p-3.5 rounded-xl border border-gray-100 dark:border-gray-800">
                      ...
                      <div className="h-3 bg-gray-200/60 dark:bg-gray-700/60 rounded-full overflow-hidden border border-gray-200/40 dark:border-gray-700/40 relative">
                        <div
        ...
        return (
          <div key={item.id} className="bg-white dark:bg-[#151D2A] p-6 rounded-3xl shadow-md border border-gray-100 dark:border-gray-800 space-y-4 hover:shadow-lg transition-all">
            {item.alert_message && (
              <div className={`p-3.5 rounded-2xl text-xs font-bold transition-all border shadow-sm ${
                item.status === "red" ? "bg-red-50 dark:bg-red-950/40 border-red-200 dark:border-red-900/50 text-red-700 dark:text-red-300" :
                item.status === "amber" ? "bg-amber-50 dark:bg-amber-950/40 border-amber-200 dark:border-amber-900/50 text-amber-700 dark:text-amber-300" :
                "bg-green-50 dark:bg-green-950/40 border-green-200 dark:border-green-900/50 text-green-700 dark:text-green-300"
              }`}>
                {item.alert_message}
              </div>
            )}
            ...
            <div className="flex items-center gap-6">
              <img src={item.img} alt="Identicon" className="w-14 h-14 rounded-full bg-gray-50 dark:bg-[#1E293B] border-2 border-gray-100 dark:border-gray-700 object-cover flex-shrink-0" />
  ...
  {showModal && (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white dark:bg-[#151D2A] w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800 animate-in fade-in zoom-in-95 duration-150 max-h-[92vh] flex flex-col">
        ...
        <form onSubmit={handleSubmitBudget} className="flex flex-col h-full overflow-hidden">
          <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3 bg-blue-50/40 dark:bg-blue-950/20 p-3.5 rounded-2xl border border-blue-100/70 dark:border-blue-900/40 animate-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1 custom-scrollbar">
                    {formData.buckets.map((bucket, index) => (
                      <div key={index} className="flex flex-col gap-2 bg-white dark:bg-[#1E293B] p-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                        <div className="flex gap-2 items-center">
                          ...
                          <div className="relative w-24 flex-shrink-0">
                            <input
                              ...
                              className="w-full bg-gray-50 dark:bg-[#151D2A] text-gray-800 dark:text-gray-100 border border-gray-100 dark:border-gray-700 p-2 text-xs font-black text-right pr-6 rounded-lg outline-none focus:border-blue-400"
                              required
                            />
                          ...
                          <button
                            ...
                            className="p-2 text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                          >
                                  ...
                                  <button
                                    ...
                                    className={`text-[9px] px-2 py-1 rounded-full border transition-colors cursor-pointer ${
                                      isSelected
                                        ...
                                        : 'bg-white dark:bg-[#151D2A] text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'
                                    }`}
                                  >
                                    {cat.name}
                                  </button>
                          ...
                          return (
                            <div key={cat.id}
                                 ...
                                 className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-all border ${
                                   ...
                                   isChecked
                                     ? 'bg-white dark:bg-[#151D2A] border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 shadow-sm font-bold'
                                     : 'border-transparent bg-white/40 dark:bg-[#151D2A]/40 text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-[#151D2A] hover:shadow-sm'
                                 }`}
                            >
                              {(formData.is_group_budget && formData.strategy_type === "spending_cap") ? (
                                isChecked ? <FaCheckSquare className="text-green-500 text-sm flex-shrink-0" /> : <FaSquare className="text-gray-300 dark:text-gray-600 text-sm flex-shrink-0" />
                              ) : (
                                isChecked ? (
                                  <div className="w-3.5 h-3.5 rounded-full border-4 border-green-500 bg-white dark:bg-[#151D2A] flex items-center justify-center flex-shrink-0"></div>
                                ) : (
                                  <div className="w-3.5 h-3.5 rounded-full border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-[#151D2A] flex-shrink-0"></div>
                                )
                              )}

                              <span className={`text-xs capitalize ${cat.isSub ? 'text-gray-500 dark:text-gray-400 font-medium' : 'font-bold text-gray-800 dark:text-gray-100'}`}>
                                {cat.isSub && <span className="text-gray-300 dark:text-gray-600 mr-1.5 font-mono">↳</span>}
                                {cat.name}
                              </span>
                            </div>
                          );
                ...
                <div className="space-y-1 animate-in fade-in duration-100">
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="p-3 text-xs border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#1E293B] text-gray-800 dark:text-gray-100 rounded-xl outline-none focus:bg-white dark:focus:bg-[#151D2A] font-black"
                    >
                    ...
                    <div className="relative flex-1 flex items-center">
                      <input
                        ...
                        required={formData.strategy_type !== "master_allocation"}
                      />
                      <span className="absolute left-3 text-gray-400 dark:text-gray-500 font-bold text-sm"><FaDollarSign size={12} /></span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {formData.strategy_type === "spending_cap" && (
                <div className="flex items-center justify-between p-3 bg-purple-50/50 dark:bg-purple-950/30 rounded-2xl border border-purple-100/60 dark:border-purple-900/40 cursor-pointer select-none animate-in fade-in duration-100"
                ...
                >
                  ...
                  <input
                    ...
                    className="w-4 h-4 text-purple-600 rounded border-purple-200 dark:border-purple-800 focus:ring-purple-400 cursor-pointer"
                  />
                </div>
              )}
            </div>
          ...
          <div className="flex justify-end gap-4 p-5 bg-gray-50 dark:bg-[#1E293B] border-t border-gray-100 dark:border-gray-800 font-bold text-xs mt-auto">
            <button
              type="button"
